...
func TestEntityTreeImpl_GetSelectionActions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			actions := tree.GetSelectionActions(tt.selectedEntity)
...
func TestEntityTreeImpl_UpdatePrettyPrintPrefixes_Simple(t *testing.T) {
	...
	tree.UpdatePrettyPrintPrefixes()
	...
	// all entities should have tree-like prefixes now
	expectedPrefixes := []string{"", "  ", "  └─", "    └─", "      └─", "", "  ", "  └─", "    └─", "      └─"}
	for i, e := range entities {
		if e.Prefix != expectedPrefixes[i] {
			t.Errorf("Expected prefix to be %q, got %q for entity %d", expectedPrefixes[i], e.Prefix, i)
		}
	}
}
