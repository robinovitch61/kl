...
func (m *Model[T]) scrollSoSelectionInView() {
	...
	if numLinesInSelection != numLinesOfSelectionInView {
		if m.topItemIdx < m.selectedItemIdx {
			...
			m.scrollUp(max(0, m.getNumContentLines()-numLinesInSelection))
		} else {
...
func (m Model[T]) getVisibleContentLines() visibleContentLinesResult {
	...
	renderAndHighlight := func(item T, idx int) string {
		...
		// TODO: do this accounting for existing ansi color codes
		return strings.ReplaceAll(item.Render(), m.stringToHighlight, m.HighlightStyle.Render(m.stringToHighlight))
	}

	if m.wrapText {
		itemLines := wrap(renderAndHighlight(currItem, currItemIdx), m.width)
...
func (m Model[T]) numLinesOfSelectionInView() int {
	if !m.selectionEnabled {
		panic("numLinesOfSelectionInView called when selection is disabled")
	}
	visibleContentLines := m.getVisibleContentLines()
	res := 0
...
func (m Model[T]) styleSelection(s string) string {
	...
	for i, section := range split {
		if section != "" {
			finalResult += m.SelectedContentStyle.Render(section)
		}
		if i < len(split)-1 {
			finalResult += re.FindStringSubmatch(s)[0]
			s = s[len(re.FindStringSubmatch(s)[0]):]
		}
	}
	return finalResult
}
